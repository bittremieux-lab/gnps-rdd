...
def test_RDD_counts_to_wide(rdd_counts_instance):
    ...
    simulated_RDD_counts = pd.DataFrame({
        "filename": ["file_samp_1.mzXML", "file_samp_2.mzXML", "file_samp3.mzXML", "file_samp4.mzXML"],
    ...
    expected_wide_format = pd.DataFrame({
        ...
        "group": ["G1", "G1", "G1", "G1"],
    }, index=["file_samp_1.mzXML", "file_samp_2.mzXML", "file_samp3.mzXML", "file_samp4.mzXML"])
...
def test_invalid_metadata_format():
    with pytest.raises(ValueError, match="External metadata file must be a CSV, TSV, or TXT."):
        _load_RDD_metadata("invalid_file_format.doc")

def test_missing_metadata_file():
    with pytest.raises(FileNotFoundError, match="External metadata file 'missing_file.csv' not found."):
