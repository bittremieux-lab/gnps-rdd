...
@pytest.mark.parametrize("reference_types, level, expected_count", [
    (["complex"], 1, 1),
    (["fruit"], 2, 1),
    (None, 1, 2),
])
def test_filter_counts(mock_gnps_and_metadata, reference_types, level, expected_count):
    ...
    # Filter counts
    filtered_counts = rdd_counts.filter_counts(reference_types=reference_types, level=level)
    
    # Validate filtering logic
    assert not filtered_counts.empty, "Filtered counts should not be empty"
    assert sum(filtered_counts['count']) == expected_count
    if reference_types:
        assert all(filtered_counts["reference_type"].isin(reference_types)), "Filtered types mismatch"
...
def test_update_groups(mock_gnps_and_metadata, tmp_path):
    ...
    # Update groups
    rdd_counts.update_groups(metadata_file=str(updated_metadata_path), merge_column="new_group")
    print(rdd_counts.sample_metadata)
    # Validate group updates
    assert all(rdd_counts.counts["group"].isin(["group1", "group2"])), "Group update failed"
...
def test_generate_RDDflows(mock_gnps_and_metadata):
    ...
    assert "source" in flows.columns and "target" in flows.columns and "value" in flows.columns
